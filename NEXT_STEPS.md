...
- **Wallet connect never logs in.** `WalletContext.connect()` only calls Freighter's `isConnected`/`getPublicKey` — it never calls `authApi.login`. `localStorage`'s `tt_token` is never written anywhere in the repo, so every backend request goes out with no `Authorization` header. (This pairs with techtown-payroll-backend's auth gaps — see that repo's NEXT_STEPS.md — but the frontend doesn't even attempt to use auth today.) See ISSUES.md #28.
